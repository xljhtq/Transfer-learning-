...
class Base_model(object):
    ...
    def adversarial_loss(self):
        ...
        logits = tf.nn.softmax(logits, name='logits_prob')
        adv_losses = self.general_loss(logits=logits,
                                       labels=self.one_hot_input_task)
        loss_adv = tf.reduce_mean(adv_losses)
...
class MTLModel_0(object):
    ...
    def general_func(self):
        ...
        general_losses = tf.nn.softmax_cross_entropy_with_logits(logits=general_feature_linear,
                                                                 labels=self.objects.input_y)
...
class MTLModel_1(object):
    ...
    def specfic_func(self):
        ...
        #                                    labels=self.objects.input_y)
        specfic_losses = tf.nn.softmax_cross_entropy_with_logits(logits=specfic_feature_linear,
                                                                 labels=self.objects.input_y)
